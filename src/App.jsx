...
const App = () => {
  ...
  const handleSubmit = () => {
    ...
    const updatedEducationalErrors = educationalForms.map((form, index) => {
      ...
      if (form.totalMarks !== "" && form.marksObtained !== "" && Number(form.totalMarks) < Number(form.marksObtained)) {
        errors.totalMarks = "Total Marks must be greater than or equal to Marks Obtained.";
      }
    ...
    if (Object.keys(personalErrors).length === 0 && updatedEducationalErrors.every(errors => Object.keys(errors).length === 0)) {
